...
#[tauri::command]
async fn ls(manager: State<'_, SessionManager>, device: Device, path: String) -> Result<Vec<FileItem>, Error> {
  if !path.starts_with("/") {
    return Err(Error::new("Absolute path required"));
  }
  let mut entries: Vec<String> = String::from_utf8(manager.exec(device.clone(), "xargs -0 -I PATH find PATH -maxdepth 1 -print0",
                                                                Some(path.clone().into_bytes())).await?).unwrap().split('\0')
    .map(|l| String::from(l))
    .collect();
  ...
  for chunk in entries.chunks(100) {
    ...
    let mut details: Vec<String> = String::from_utf8(manager.exec(device.clone(), "xargs -0 ls -ld --full-time", Some(ls_input)).await?).unwrap().split('\n')
      .map(|l| String::from(l))
      .collect();
    // Last line is empty, remove it
    details.pop();
    assert_eq!(chunk.len(), details.len());
    let mut group: Vec<FileItem> = zip(chunk, details).skip(1).map(|(entry, line)| {
...
pub fn plugin<R: Runtime>(name: &'static str) -> TauriPlugin<R> {
  Builder::new(name)
    .invoke_handler(tauri::generate_handler![ls, read, write, get, put, get_temp])
    .build()
}
...
impl FileItem {
  ...
  fn new(path: &str, entry: &str, line: &str) -> FileItem {
    ...
    let info_cols: Vec<&str> = line[..info_name_index - 1].split_ascii_whitespace().collect();
    let perm = *info_cols.get(0).unwrap();
    let file_type = String::from(&perm[..1]);

    let size = if file_type == "-" { info_cols[4].parse::<usize>().unwrap() } else { 0 };
    ...
    return FileItem {
      ...
      mtime: format!("{}T{}{}", info_cols.get(info_cols.len() - 3).unwrap(),
