...
fn devices_file_path() -> Result<PathBuf, IoError> {
  return match env::var("APPDATA").or_else(|_| env::var("HOME"))
    .or_else(|_| env::var("USERPROFILE")) {
    Ok(val) => Ok(PathBuf::from(val).join(".webos").join("ose")
      .join("novacom-devices.json")),
    _err => Err(IoError::new(ErrorKind::NotFound, "Failed to open data path"))
  };
}
